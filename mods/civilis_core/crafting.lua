...
local function get_selection_formspec()
	...
	for i, recipe in ipairs(civ.recipes) do
		...
		if data:get_string(recipe[2]) ~= "" then 
			y_offset = (i-1)*1.1
			formspec = formspec
			--.."box[0.5,"..minetest.serialize(y_offset)..";6,2;#9499a1]"
			.."item_image[0.5,"..y_offset..";1,1;"..recipe[2].."]"
			.."button[1.5,"..y_offset..";5,1;"..recipe[2]..";Craft "..recipe[1].."]"
		end
	end
	return formspec
end
...
local function get_crafting_formspec(recipe)
	...
	for i, requirement in ipairs(recipe[3]) do
		...
		y_offset = (i+1)*1
		formspec = formspec
		.."item_image[0.5,"..y_offset..";1,1;"..stack:get_name().."]"
		.."label[1.5,"..y_offset..";x"..stack:get_count().."]"
	end
...
local function get_materials_formspec()
	...
	for i, material in ipairs(civ.materials) do
		if (basemeta:get_float(material.."rate") ~= 0) and (basemeta:get_float(material.."rate") ~= "") then
			y_offset = (i-1)*1.1
			formspec = formspec..
				"item_image[1,"..y_offset..";1,1;"..material.."]"..
				"label[2,".. y_offset+0.2 ..";produced per a second: "..basemeta:get_float(material.."rate").."]"
		end
	end
	return formspec
end
	...
	for i, recipe in ipairs(civ.recipes) do
		...
		if data:get_string(recipe[2]) == "" then 
			...
			if flag then 
				...
				formspec = formspec
				.."item_image[0.5,"..y_offset..";1,1;"..recipe[2].."]"
				.."button_exit[1.5,"..y_offset..";5,1;"..recipe[2].."_research;Unlock "..recipe[1].." (costs "..recipe[5].." research)]"
			end
		end
	end
	return formspec
...
-- so hacky and unreliable, there's probably a better way to get a timer in a formspec. 
minetest.register_node(c.."timer", {
	...
	on_timer = function(pos)
		...
		local formspec = 
		"size[9,5]"..
		"image[0.5,0.5;8,1;(progress_bar_background.png^[lowpart:".. (crafttime/totaltime)*100 ..":civ_progress_bar.png)^[transformR270 ]"
...
sfinv.register_page(c.."resources", {
	...
	get = function(self, player, context)
		return sfinv.make_formspec(player, context,
			"box[0.05,0.05;8,4.9;grey]".. get_materials_formspec()
		, true)
	end
})
...
sfinv.register_page(c.."structcraft", {
	...
	get = function(self, player, context)
		local basemeta = minetest.get_meta(minetest.deserialize(data:get_string("basepos")))
		return sfinv.make_formspec(player, context,
			"box[0.05,0.05;8,4.9;grey]"..
			"scrollbaroptions[min=0;max=300;largestep=100,smallstep=10]"..
			"scrollbar[0,0;0.5,5;vertical;scrollbar;0]"..
			"scroll_container[0.2,0.4;11,5.7;scrollbar;vertical]"..
				get_selection_formspec()..
			...
			for _, recipe in ipairs(civ.recipes) do 
				if recipe[2] == field then
					...
					--this page shows resources
						...
						minetest.set_node(timerpos, {name = c.."timer"})
				...
				elseif recipe[2].."_research" == field then
					local inv = minetest.get_inventory({type="player", name=player:get_player_name()})
					if inv:contains_item("main", civ.."research "..recipe[5]) then
						data:set_string(recipe[2], "researched")
						inv:remove_item("main",civ.."research "..recipe[5])
					else
